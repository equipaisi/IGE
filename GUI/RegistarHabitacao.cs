...
namespace Mockups
{
    public partial class RegistarHabitacao : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // Código Postal
//            var cp = new IGE.CodigoPostal(textBoxCodigoPostal.Text);
//            MessageBox.Show($"O seu código postal é {cp}");
            ...
            // Comodidades
            var comodidades = new IGE.Comodidades(checkBoxTelevisao.Checked, checkBoxInternet.Checked, checkBoxServicosDeLimpeza.Checked);

            // Descrição da Habitação
            var descricao = textBoxDescricao.Text;

            // Construir a habitação
            var quartos = new List<IGE.Quarto>(numQuartos);
            for (var i = 0; i < numQuartos; i++)
            {
                quartos.Add(new Quarto(new List<ICama> {new Cama(TipoCama.Single)}));
            }
//            var morada = new IGE.Morada(null, 0, null); // TODO
//            var habitacao = new IGE.Habitacao(quartos, numAssoalhadas, metrosQuadrados, anoDeConstrucao, null, comodidades);
